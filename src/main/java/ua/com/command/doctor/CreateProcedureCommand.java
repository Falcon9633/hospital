...
public class CreateProcedureCommand implements Command {
    private static final Logger LOGGER = LogManager.getLogger(CreateProcedureCommand.class);
    ...
    public String execute(HttpServletRequest req, HttpServletResponse resp) {
        ...
        Procedure procedure = new Procedure(nameEN, nameUA, descriptionEN, descriptionUA, account.getId(), empId, medicalCardId);
        ProcedureDao procedureDao = DaoFactory.getProcedureDao();
        Procedure inserted = procedureDao.insert(procedure);
        if (inserted.getId() == null) {
            Validator.setErrorMessage(session, locale.getMessage("create_procedure_command.error.failed_to_create"), LOGGER, forward);
        }
